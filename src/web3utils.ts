...
async function uploadMetadata(file: any, name: string, symbol: string, description: string, feeBasisPoints: number) {
    ...
    try {
        ...
        fs.unlinkSync("./metadata.json");
        fs.writeFileSync('metadata.json', JSON.stringify(metadata));
        uri = `https://${data.value.cid}.ipfs.nftstorage.link`
    } catch (ex) {
        console.log(ex)
    }
...
async function uploadImageLogo(imagePath: string) {
    ...
    try {
        ...
        imageUrl = `https://${data.image.split("//")[1].split("/")[0]}.ipfs.nftstorage.link/${imageName}`;
    } catch (ex) {
        console.log(ex)
    }
    return imageUrl
...
export async function withdrawalFees(connection: Connection, payer: Keypair, mint: PublicKey, withdrawWithheldAuthority: Keypair, accountsToWithdrawFrom: PublicKey[]) {
    try {
